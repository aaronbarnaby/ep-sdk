...
export class RequestFactory {
  ...
  private async sendInternal<T>(
    ...
    headers.set('X-MOLTIN-APPLICATION', this.config.application)

    if (this.config.language) {
      headers.set('X-MOLTIN-LANGUAGE', this.config.language)
    }
    ...
    if (this.config.storeId) {
      headers.set('X-MOLTIN-AUTH-STORE', this.config.storeId)
    }
    ...
    const response = await this.config.fetch(
      `${this.config.protocol}://${this.config.host}/oauth/access_token`,
      {
        ...
        headers: {
          ...
          'X-MOLTIN-APPLICATION': this.config.application
        },
        body: body.toString()
      }
    )
  ...
  private resolveStoredCredentials(): AuthResponse | null {
    const rawCredentials = this.storage.get(
      resolveCredentialsStorageKey(this.config.name)
    )
  ...
  private async resolveAccessToken(
    ...
    if (
      cachedCredentials?.access_token &&
      !this.isTokenExpired(cachedCredentials)
    ) {
