...
describe('request factory', () => {
  it('wraps request bodies and applies auth headers from a custom authenticator', async () => {
    ...
    const response = await client.request.send<{ data: { id: string } }>('/products', 'POST', {
      body: {
    ...
    expect(requests[0]?.input).toBe('https://euwest.api.elasticpath.com/v2/products');
  ...
  it('creates and caches an access token with the legacy client credential keys', async () => {
    ...
    expect(requests[0]?.input).toBe('https://euwest.api.elasticpath.com/oauth/access_token');
    expect(requests[0]?.init?.body).toBe(
    ...
    expect(authHeaders.get('Content-Type')).toBe('application/x-www-form-urlencoded');
